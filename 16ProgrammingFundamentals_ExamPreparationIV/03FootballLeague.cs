...
namespace _03FootballLeague
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (KeyValuePair<string, long> team in teamsByGoals.OrderByDescending(t => t.Value).ThenBy(t => t.Key))
            {
                string teamAndGoalsString = $"- {team.Key} -> {team.Value}";
                listOfTeamsByGoals.Add(teamAndGoalsString);
            }
        ...
        {
            char[] teamCharArray = team.ToCharArray();
            Array.Reverse(teamCharArray);
            string reversedTeam = string.Empty;
            foreach (char ch in teamCharArray)
            {
                reversedTeam += ch;
            }
